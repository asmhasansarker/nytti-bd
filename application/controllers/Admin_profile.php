...
class Admin_profile extends CI_Controller {
    ...
    function student_details($id){
		$data['student_data'] = $this->db_model->select_data('*','students use index (id)',array('admin_id'=>$this->session->userdata('uid'),'id'=>$id));
		$data['batch_name'] = $this->db_model->select_data('id,batch_name','batches  use index (id)',array('admin_id'=>$this->session->userdata('uid'),'status'=>1, 'id'=>$data['student_data'][0]['batch_id']),'',array('id','desc'));
